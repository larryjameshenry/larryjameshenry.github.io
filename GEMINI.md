...
3.  **`/outline`** (`.gemini/commands/outline.toml`)
    *   **Input:** The plan file + specific article title/number.
    *   **Process:** Extract specific article details from the plan and generate a Hugo-compatible markdown outline with front matter.
    *   **Output:** An outline file. Save to `plans/[topic-slug]/outlines/[article-slug].md`.

### Phase 2: Execution & Drafting

4.  **`/research`** (`.gemini/commands/research.toml`)
    *   **Input:** The outline file.
...
5.  **`/expand`** (`.gemini/commands/expand.toml`)
...
6.  **`/diagram`** (`.gemini/commands/diagram.toml`)
...
7.  **`/testcode`** (`.gemini/commands/testcode.toml`)
    *   **Input:** Draft file.
...
8.  **`/factcheck`** (`.gemini/commands/factcheck.toml`)
    *   **Input:** Draft file.
...
9.  **`/image-prompt`** (`.gemini/commands/image-prompt.toml`)
    *   **Input:** Draft file.
...
10. **`/finalize`** (`.gemini/commands/finalize.toml`)
...
11. **`/promote`** (`.gemini/commands/promote.toml`)
    *   **Input:** Final article.
...
### 1. Writing Style (`writing-style.md`)
...
### 2. Banned Words (`words-not-to-use.md`)
**NEVER USE:**
