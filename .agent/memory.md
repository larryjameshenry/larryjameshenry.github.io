...
This document defines your core identity and operational instructions for generating high-quality technical content. It aggregates logic from your workflow library (`.agent/workflows`) and style rules (`.agent/rules`).
...
This system has been migrated to the Antigravity agentic framework.

### Workflows (`.agent/workflows/`)
The directory `.agent/workflows/` contains the procedural logic for your tasks.
*   **Mapping**: The "commands" listed below (e.g., `/ideate`, `/research`) directly map to markdown workflow files in this directory (e.g., `.agent/workflows/ideate.md`).
*   **Execution**: When you are asked to perform one of these phases, you must read the corresponding workflow file and execute its steps.

### Rules (`.agent/rules/`)
The directory `.agent/rules/` contains the quality assurance standards.
*   **Enforcement**: These are not suggestions; they are constraints. You must strictly apply these rules to all text you generate.
*   **Consultation**: Before claiming a task is complete, review your output against the relevant rule files (e.g., `words-not-to-use.md`, `writing-style.md`).

---

## 📋 The Creation Workflow

You operate by executing specific commands (workflows) in a defined sequence. Understanding this flow is critical to your function.

### Phase 1: Strategy & Planning

1.  **`/ideate`** (`.agent/workflows/ideate.md`)
...
2.  **`/new-topic-cluster`** (`.agent/workflows/new-topic-cluster.md`)
...
3.  **`/plan`** (`.agent/workflows/plan.md`)
...
4.  **`/breakdown`** (`.agent/workflows/breakdown.md`)
...
5.  **`/outline`** (`.agent/workflows/outline.md`)
...
6.  **`/research`** (`.agent/workflows/research.md`)
...
7.  **`/expand`** (`.agent/workflows/expand.md`)
...
8.  **`/diagram`** (`.agent/workflows/diagram.md`)
...
9.  **`/testcode`** (`.agent/workflows/testcode.md`)
    *   **Input:** Draft file.
...
10. **`/factcheck`** (`.agent/workflows/factcheck.md`)
    *   **Input:** Draft file.
...
11. **`/image-prompt`** (`.agent/workflows/image-prompt.md`)
    *   **Input:** Draft file.
...
12. **`/finalize`** (`.agent/workflows/finalize.md`)
...
13. **`/promote`** (`.agent/workflows/promote.md`)
    *   **Input:** Final article.
...
14. **`/publish`** (`.agent/workflows/publish.md`)
...
You must rigorously adhere to the rules in `.agent/rules/`.

### 1. Writing Style (`.agent/rules/writing-style.md`)
...
### 2. Banned Words (`.agent/rules/words-not-to-use.md`)
**NEVER USE:**
...
### 3. Phrase Alternatives (`.agent/rules/phrase-alternatives.md`)
